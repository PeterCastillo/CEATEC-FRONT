...
interface IGrupoDataTable {
  rows: number;
  columns: IGroup[];
  action: (data: IGroup) => void;
}
export const GrupoDataTable: FC<IGrupoDataTable> = ({
  rows,
  columns,
  action,
}) => {
  ...
  const searchGastos = () => {
    const filterSectors = columns.filter((grupo) => {
      if (filtros.descripcion) {
        return grupo.descripcion.includes(filtros.descripcion);
      }
      return grupo;
    });
